    ...
    {      
        ...
        if( pTask->hTask != BAD_RTHANDLE )
        {
            ...
            while( pTask->bBusy && --WaitCnt )
                RtSleep(10);            /* sleep 10ms to let the background task to complete its init sequence */
...
void * AllocateDpr( UINT32 PhysicalAddress, UINT32 Size )
 {
    return malloc( Size );   // for test purposes !!!
 }

int FreeDpr(void * dpr)
 {
    free( dpr );           // for test purposes !!!
	return SUCCESS;
}
...
void	InCardCopy(VOID *Dest, VOID *Source, UINT32 Size,UINT32 dType,UINT32 IOSwap)
{
	UINT16	portAddr;
    ...
    case DEVICE_1B_INPUT:
		{
			portAddr= (UINT16) Source;
			for(; Size ; Size--  ) 
				*((UINT8  volatile*) Dest)++ = inbyte( portAddr++);
			break;
		}
	...
	case DEVICE_1B_INPUT_G:
	  *((UINT8  volatile*)Dest) = *((UINT8  volatile*)Source);
		break;
	}

}

void	OutCardCopy(VOID *Dest, VOID *Source, UINT32 Size, UINT32 dType, UINT32 IOSwap)        

{
    ...
    switch( dType )
    {
		...
		{
		portAddr= (UINT16) Dest;
		for(; Size ; Size--  ) 
		// outbyte( *((UINT8  volatile*) Dest)++ , *((UINT8  volatile*) Source)++);
		outbyte( portAddr++ , *((UINT8  volatile*) Source)++);
		break;	
		}
	...
	void	ZeroCardCopy(VOID *Dest, VOID *Source, UINT32 Size, UINT32 dType)        

{
    ...
    case DEVICE_4W_OUTPUT_G:
    for(; Size >= 4; Size -= 4) *((UINT32 volatile*)Dest)++ = (UINT32) 0;
	break;

    case DEVICE_2W_OUTPUT_G:
    for(; Size >= 2; Size -= 2) *((UINT16 volatile*)Dest)++ = (UINT16) 0;

    case DEVICE_1W_OUTPUT_G:
	    for(; Size     ; Size--   ) *((UINT8  volatile*)Dest)++ = (UINT8)  0;
	break;

    case DEVICE_1B_OUTPUT_G:
     *((UINT8  volatile*)Dest) = (UINT8) 0;
	break;
	}
