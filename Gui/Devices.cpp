...
int CDevice::LoadDevice()
{
    ...
    if( rc == SUCCESS )
    {
          ...
          )
        {
            ...
            m_DeviceInst.StnAddress    = DeviceCfg.StnAddress;
            m_DeviceInst.bCritical  = (UINT8)DeviceCfg.bCritical;
            m_DeviceInst.Type       = m_DevType;
			m_DeviceInst.IOAddress	= DeviceCfg.IOAddress;
			m_DeviceInst.StnAddress	= DeviceCfg.StnAddress;
			m_DeviceInst.IOSwap		= DeviceCfg.IOSwap;
...
int CDevice::Load( UINT32 NameOfs, UINT32 InputOfs, UINT32 OutputOfs )
{
    ...
    if( rc == SUCCESS )
    {
        ...
        if( m_arrInput.GetSize() )                  // if there are any input points defined here
        {
			...
			m_Input.IOSwap		= m_DeviceInst.IOSwap;
			m_Input.pSrc		= (void*) m_DeviceInst.IOAddress;
			m_Input.pDst		= (void*)m_DeviceInst.IOAddress;
			m_Input.IODpr		= m_DeviceInst.IOAddress;
			m_Input.Type		= m_DeviceInst.Type ;
			m_Input.StnAddress	= m_DeviceInst.StnAddress;		// funny did not appear in dev_io
        ...
        if( m_arrOutput.GetSize() )                 // if there are any output points defined here
        {
			...
			m_Output.IOSwap		= m_DeviceInst.IOSwap;
			m_Output.pDst		= (void*)m_DeviceInst.IOAddress;
			m_Output.pSrc		= (void*)m_DeviceInst.IOAddress;
			m_Output.IODpr		= m_DeviceInst.IOAddress;
			m_Output.Type		= m_DeviceInst.Type;
			m_Output.StnAddress	= m_DeviceInst.StnAddress; 	// funny did not appear in dev_io
...
int CDevice::Check( CDevice* pDevice )      // makes sure "this" can live with pDevice
{
    ...
    if( m_DeviceInst.StnAddress == pDevice->m_DeviceInst.StnAddress )
    {
...
int CDeviceArray::Check( CDevice* pDevice )
{
    ...
    for( int DevIndex = 0; DevIndex < GetSize() && (rc == SUCCESS) ; DevIndex++ )
    {
        ...
        if( rc1 != SUCCESS )
        {
            Erop( rc1, pDevice->m_pName, pDev->m_pName, m_pNetName, pDevice->m_DeviceInst.StnAddress );
            rc = FAILURE;
        }
    }
